...
as a random sparse pattern on LD label lines of its own, concatenated to layer 1's lines.
...
K_ON = 10              # lines on in each class's random label pattern (of LD)
LD = 100               # label lines
...
class Tower:
    def __init__(self, rng, ink_freq):
        ...
        self.label_w = 1.0
        self.pat = np.zeros((10, LD), np.float32)                      # one random sparse pattern per class, on the label lines
        for c in range(10):
            self.pat[c, rng.choice(LD, K_ON, replace=False)] = 1.0
        freq2 = np.concatenate([np.full(H1, RHO1), np.full(LD, K_ON / LD)]).astype(np.float32)   # a label line is on in about K/LD of images
        self.L2 = Layer(H1 + LD, H2, RHO2, rng, freq2, d_code=H1)

    def settle(self, X, y=None, beta=BETA):
        B = len(X)
        lab = self.label_w * self.pat[y] if y is not None else np.zeros((B, LD), np.float32)
        ...
        for _ in range(ITERS):
            ...
            z = np.concatenate([a1, lab], 1)                                 # the label pattern on its own lines
            b2 = z @ W2.T
    ...
    def generate(self):
        ...
        z = np.concatenate([np.zeros((10, H1), np.float32), self.label_w * self.pat], 1)
        b2 = z @ self.L2.Wd.T
...
def main():
    ...
    match = W2[:, H1:] @ tw.pat.T                                            # weight each cell puts on each class pattern
    ...
    tag = f"_cat_p{passes}_{passes2}_k{K_ON}" + ("_share" if share2 else "")
