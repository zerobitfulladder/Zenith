...
import sys
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "competitive"))
from compete import geo_step as geo_batch          # correct multi-sample tangent
...
FREEZE_SIT = True     # let ONLY the motor half adapt: the expert's
                      # territory is set by the seeding and held fixed,
                      # so consolidating a policy cannot drag the gate
...
def main():
    ...
    for it in range(BATCHES):
        ...
        for h in range(H):                              # keep the best slice
            ...
            if (adv[np.argsort(-adv)[:len(keep)]] > 0).any():
                Wn = geo_batch(bank.W[h], XS[keep], ETA_C)   # not the mean of them
                if FREEZE_SIT:
                    Wn[:, :DIM - N_MOT * 12] = bank.W[h][:, :DIM - N_MOT * 12]
                    Wn /= np.linalg.norm(Wn, axis=1, keepdims=True) + EPS
                bank.W[h] = Wn
    ...
    np.savez_compressed(OUT / "cem2.npz", W=bank.W.astype(np.float32),
                        V=bank.V, uses=bank.uses)
    (OUT / "cem2.json").write_text(json.dumps(
        {"kind": "module", "module": "drone_cem2",
         "dir": "experiments/2026_08_31/drone_rl", "m_cand": 5}))
    (OUT / "cem2_metrics.json").write_text(json.dumps(
        {"config": {"H": H, "K": K, "gamma": GAMMA, "topq": TOPQ, "freeze_situation": FREEZE_SIT,
    ...
    fig.tight_layout(); fig.savefig(OUT / "03_cem_frozen.png", dpi=130); plt.close(fig)
