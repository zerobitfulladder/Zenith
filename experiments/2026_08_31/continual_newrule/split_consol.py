...
TAG = ("consol" if CONSOL else "plain") + ("" if REPEL else "_norepel")
...
def main():
        ...
        for ep in range(EPOCHS):
            ...
            for s in range(0, len(o), BATCH):
                ...
                if REPEL and seen > 0:             # no repulsion in the 1st epoch ever
                    ...
                    plastic = np.minimum(1.0, f * H) if CONSOL else np.ones(H)
                    for h in np.unique(fw[bad]):
                        p = bad & (fw == h)
                        if p.sum() >= MIN_S and plastic[h] > 1e-3:
                            W[h] = geo_step_neg(W[h], Q[p], ETA_NEG * plastic[h],
                                                CAP_NEG)
                            n_rep += int(p.sum())
            seen += 1
    ...
    print(f"plasticity at the end: "
          f"{np.array2string(np.minimum(1.0, f * H), precision=2, floatmode='fixed')}")
