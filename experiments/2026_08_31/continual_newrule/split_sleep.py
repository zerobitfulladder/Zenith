"""The forgetting experiment with sleep in place of punishment.
...
The only change is HOW the learner is chosen and corrected:
    ...
    correct  = the winner learns the joined vector. NOTHING is punished on real
               data. After every epoch the population sleeps: each hypercolumn
               dreams from its own coefficient statistics and the nearest rival
               of another class is pushed off the dream. No data, no teacher,
               so a new class cannot aim the correction at the old experts.
...
import sleepwake as sw                                             # noqa: E402
...
BASE = {"logistic (SGD)": (0.9487, 0.0094), "MLP 256 (SGD)": (0.9765, 0.0000),
        ...
        "new rule, repulsion": (0.9828, 0.3171),
        "new rule, no correction": (0.9800, 0.9620)}
...
def calibrate(E, y, claim, alive, b, rng):
...
def main():
    ...
    for ph, digits in (("1", PH1), ("2", PH2)):
        ...
        for ep in range(EPOCHS):
            ...
            seen += 1
            n, _ = sw.sleep(W, wins, coef, rng, **NIGHT)
            n_rep += n
            print(f"  phase {ph} epoch {ep+1}/{EPOCHS}   sleep corrections {n_rep}",
                  flush=True)
