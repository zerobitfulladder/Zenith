"""Rich palette / sparse speech: does a big L1 dictionary rescue sparse output?

2x3 grid: K1 in {36, 512} x L1 output in {dense, top3, top1 per position}.
L1 learning unchanged (dense-window top-1 winner); L2/L3 skeleton learning,
L4 dense — all standard. Predictions in README.md before running.

Run:  GF_W1_STR=1 .venv/bin/python experiments/2026_08_24/rich_palette/run_rich_palette.py
"""
...
OUTPUT_DIR = ROOT / "experiments" / "2026_08_24" / "rich_palette" / "results"
EVAL_B = 250
CONFIGS = [(36, "dense"), (36, "top3"), (36, "top1"),
           (512, "dense"), (512, "top3"), (512, "top1")]


def topk_mask(maps, k):
    """Per-position: keep the k largest channels, zero the rest.
    k iterated argmaxes — cupy argpartition allocates GB-scale workspaces."""
    N, g, _, K = maps.shape
    if k >= K:
        return maps
...
def l1_out(xb, d1, mode, learning):
    m1 = level_pass(xb[..., None], d1, POS1, W1_WIN, 1, G1, False, learning)
    if mode == "top3":
...
def run_config(k1, mode, Xtr, ytr, Xte, yte, labels_x):
    ...
    # ---- Encode (mask applied), probes, hard readout ----------------------
    def encode(X):
        C2s, C3s = [], []
    ...
    # ---- Diagnostics: L1 crowding + top1-top2 margin ----------------------
    W1n = to_np(d1.W)
    Cc = W1n @ W1n.T
    crowd = float(np.abs(Cc[~np.eye(len(Cc), dtype=bool)]).mean())

    xb = xp.asarray(Xte[:EVAL_B], dtype=DTYPE)
    Vf = _windows(xb[..., None], POS1, W1_WIN)
    Vh, ok = _center_norm_rows(Vf.reshape(-1, W1_WIN * W1_WIN))
    ...
    b1, b2, b3 = Bank(W1n), Bank(to_np(d2.W)), Bank(to_np(d3.W))
    ...
    for j, ax in enumerate(axes.flat):
        ...
        ax.axis("off")
    fig.suptitle(f"Generation — K1={k1}, L1 output {mode}")
    fig.tight_layout()
...
def main():
    ...
    lines += ["", "Figures: generation_K{36,512}_{dense,top3,top1}.png"]
