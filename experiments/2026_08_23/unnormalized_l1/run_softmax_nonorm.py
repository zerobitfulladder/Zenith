"""Softmax-sampled, unnormalized L1: can stochastic selection replace
normalization as the anti-monopoly mechanism?
...
Run:  .venv/bin/python experiments/2026_08_23/unnormalized_l1/run_softmax_nonorm.py
"""
...
OUTPUT_DIR = ROOT / "experiments" / "2026_08_23" / "unnormalized_l1" / "results" / "softmax"
# ---------------------------------------------------------------------------
...
class SoftmaxUnit:
    ...
    def learn(self, x):
        if self.n_boot < self.k:
            ...
            return
        p = self.probs(x)
        j = int(self.rng.choice(self.k, p=p))
...
def encode_batch(X2d, unit):
    ...
    for pi, (r, c) in enumerate(POS):
        ...
        Z = P @ unit.W.T / unit.tau
        Z -= Z.max(axis=1, keepdims=True)
        Pr = np.exp(Z)
        Pr /= Pr.sum(axis=1, keepdims=True)
        code[:, pi * K1:(pi + 1) * K1] = (Pr * ok[:, None]).astype(np.float32)
    return code
